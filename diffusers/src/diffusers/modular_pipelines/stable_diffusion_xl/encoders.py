...
class StableDiffusionXLInpaintVaeEncoderStep(ModularPipelineBlocks):
    ...
    def intermediate_outputs(self) -> List[OutputParam]:
        return [
            ...
            OutputParam(
                ...
                type_hint=torch.Tensor,
                description="The masked image latents to use for the inpainting process (only for inpainting-specified unet)",
            ),
            OutputParam(
                "crops_coords",
    ...
    def prepare_mask_latents(self, components, mask, masked_image, batch_size, height, width, dtype, device, generator):
