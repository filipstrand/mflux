...
class LeditsAttentionStore:
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str, editing_prompts, OnP=False):
        # attn.shape = batch_size * head_size, seq_len query, seq_len_key
        if attn.shape[1] <= self.max_size:
            bs = 1 + int(OnP) + editing_prompts
            skip = 2 if OnP else 1  # skip OnP & unconditional
...
class LEditsPPPipelineStableDiffusionXL(
    ...
    def prepare_unet(self, attention_store, OnP: bool = False):
        attn_procs = {}
...
# Copied from diffusers.pipelines.ledits_pp.pipeline_leditspp_stable_diffusion.compute_noise_sde_dpm_pp_2np
def compute_noise_sde_dpm_pp_2np(scheduler, prev_latents, latents, timestep, noise_pred, eta):
...
def compute_noise(scheduler, *args):
    if isinstance(scheduler, DDIMScheduler):
        ...
        return compute_noise_sde_dpm_pp_2np(scheduler, *args)
    else:
