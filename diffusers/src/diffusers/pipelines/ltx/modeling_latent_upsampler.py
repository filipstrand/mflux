...
class PixelShuffleNP(torch.nn.Module):
    ...
    def forward(self, x):
        if self.dims == 3:
            ...
            return x.unflatten(1, (-1, *self.upscale_factors[:2])).permute(0, 1, 4, 2, 5, 3).flatten(4, 5).flatten(2, 3)
        elif self.dims == 1:
...
class LTXLatentUpsamplerModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        ConvNp = torch.nn.Conv2d if dims == 2 else torch.nn.Conv3d

        self.initial_conv = ConvNp(in_channels, mid_channels, kernel_size=3, padding=1)
        ...
        self.final_conv = ConvNp(mid_channels, in_channels, kernel_size=3, padding=1)
