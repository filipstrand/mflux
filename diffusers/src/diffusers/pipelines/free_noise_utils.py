...
class AnimateDiffFreeNoiseMixin:
        ...
        for motion_module in block.motion_modules:
            ...
            for i in range(num_transformer_blocks):
                if isinstance(motion_module.transformer_blocks[i], FreeNoiseTransformerBlock):
                    ...
                    basic_transformer_block = motion_module.transformer_blocks[i]

                    motion_module.transformer_blocks[i] = FreeNoiseTransformerBlock(
                        dim=basic_transformer_block.dim,
                        num_attention_heads=basic_transformer_block.num_attention_heads,
                        attention_head_dim=basic_transformer_block.attention_head_dim,
                        dropout=basic_transformer_block.dropout,
                        cross_attention_dim=basic_transformer_block.cross_attention_dim,
                        activation_fn=basic_transformer_block.activation_fn,
                        attention_bias=basic_transformer_block.attention_bias,
                        only_cross_attention=basic_transformer_block.only_cross_attention,
                        double_self_attention=basic_transformer_block.double_self_attention,
                        positional_embeddings=basic_transformer_block.positional_embeddings,
                        num_positional_embeddings=basic_transformer_block.num_positional_embeddings,
                    ...
                    motion_module.transformer_blocks[i].load_state_dict(
                        basic_transformer_block.state_dict(), strict=True
                    )
                    motion_module.transformer_blocks[i].set_chunk_feed_forward(
                        basic_transformer_block._chunk_size, basic_transformer_block._chunk_dim
                    )
    ...
    def _disable_free_noise_in_block(self, block: Union[CrossAttnDownBlockMotion, DownBlockMotion, UpBlockMotion]):
        ...
        for motion_module in block.motion_modules:
            ...
            for i in range(num_transformer_blocks):
                if isinstance(motion_module.transformer_blocks[i], FreeNoiseTransformerBlock):
                    free_noise_transformer_block = motion_module.transformer_blocks[i]

                    motion_module.transformer_blocks[i] = BasicTransformerBlock(
                        dim=free_noise_transformer_block.dim,
                        num_attention_heads=free_noise_transformer_block.num_attention_heads,
                        attention_head_dim=free_noise_transformer_block.attention_head_dim,
                        dropout=free_noise_transformer_block.dropout,
                        cross_attention_dim=free_noise_transformer_block.cross_attention_dim,
                        activation_fn=free_noise_transformer_block.activation_fn,
                        attention_bias=free_noise_transformer_block.attention_bias,
                        only_cross_attention=free_noise_transformer_block.only_cross_attention,
                        double_self_attention=free_noise_transformer_block.double_self_attention,
                        positional_embeddings=free_noise_transformer_block.positional_embeddings,
                        num_positional_embeddings=free_noise_transformer_block.num_positional_embeddings,
                    ).to(device=self.device, dtype=self.dtype)

                    motion_module.transformer_blocks[i].load_state_dict(
                        free_noise_transformer_block.state_dict(), strict=True
                    )
                    motion_module.transformer_blocks[i].set_chunk_feed_forward(
                        free_noise_transformer_block._chunk_size, free_noise_transformer_block._chunk_dim
                    )
