...
class DepthUtil:
    ...
    def get_or_create_depth_map(
        ...
        # 1. If depth map path is provided, use it directly
        if depth_map_path:
        ...
        # 2. Generate depth map from the image using the DepthPro model
        depth_result = depth_pro(image_path)

        # 3. Save the depth map to a file with same name + _depth suffix
        generated_depth_path = image_path.rsplit(".", 1)[0] + "_depth.png"
