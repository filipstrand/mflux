...
class QwenImageVAE(nn.Module):
    """
    Complete Qwen Image VAE based on execution trace analysis.
    """

    def __init__(self):
        super().__init__()
        # Post-quantization convolution: 1x1x1 conv to transform latents before decoder
        self.post_quant_conv = QwenImageCausalConv3D(16, 16, 1, 1, 0)  # 1x1x1 kernel, no padding
        self.decoder = QwenImageDecoder3D()

    def decode(self, latents: mx.array) -> mx.array:
        """
        Decode latents to images using traced architecture.

        Args:
            latents: Input latents [1, 16, 1, 16, 16]
        Returns:
            Decoded images [1, 3, 1, 128, 128]
        """
        # Apply post-quantization convolution first (this was missing!)
