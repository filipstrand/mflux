"""
Joint transformer block for LongCat model.

Same architecture as FLUX joint transformer blocks with bi-directional
attention between image and text modalities.
"""
...
class LongCatJointTransformerBlock(nn.Module):
    """
    Joint transformer block for LongCat.

    Performs bi-directional attention between image hidden states and
    text encoder hidden states, followed by feed-forward networks.

    This is architecturally identical to FLUX joint blocks but exists
    as a separate class for model organization.
    """
    ...
    def __init__(self, layer: int):
        ...
        self.norm2_context = nn.LayerNorm(dims=3072, eps=1e-6, affine=False)
    ...
    def __call__(
        ...
        """
        Forward pass through joint transformer block.

        Args:
            hidden_states: Image hidden states
            encoder_hidden_states: Text encoder hidden states
            text_embeddings: Combined time and text embeddings
            rotary_embeddings: Rotary position embeddings

        Returns:
            Tuple of (encoder_hidden_states, hidden_states) after attention and FFN
        """
        ...
        hidden_states = LongCatJointTransformerBlock._apply_norm_and_feed_forward(
            hidden_states=hidden_states,
        ...
        encoder_hidden_states = LongCatJointTransformerBlock._apply_norm_and_feed_forward(
            hidden_states=encoder_hidden_states,
    ...
    def _apply_norm_and_feed_forward(
        ...
        """Apply normalization, gating, and feed-forward."""
