"""
Single transformer block for LongCat model.

Same architecture as FLUX single transformer blocks with self-attention
on concatenated image and text features.
"""
...
class LongCatSingleTransformerBlock(nn.Module):
    """
    Single transformer block for LongCat.

    Performs self-attention on concatenated image and text features,
    followed by a parallel MLP branch that is projected out together
    with the attention output.

    This is architecturally identical to FLUX single blocks but exists
    ...
    def __call__(
        ...
        """
        Forward pass through single transformer block.

        Args:
            hidden_states: Concatenated image and text hidden states
            text_embeddings: Combined time and text embeddings for conditioning
            rotary_embeddings: Rotary position embeddings

        Returns:
            Updated hidden states after attention and projection
        """
    ...
    def _apply_feed_forward_and_projection(
        ...
        """Apply parallel MLP and project combined output."""
