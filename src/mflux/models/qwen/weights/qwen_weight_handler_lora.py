...
class QwenLoRALinear(nn.Module):
    def __init__(
        ...
        self.lora_A = mx.random.uniform(
            low=-std, high=std, shape=(input_dims, r)
        )
        self.lora_B = mx.random.uniform(
            low=-std, high=std, shape=(r, output_dims)
        )
    ...
    def load_lora_weights(
        ...
        for lora_file, lora_scale in zip(lora_files, lora_scales):
            try:
                ...
                transformer_weights = QwenWeightHandlerLoRA._create_qwen_lora_layers(
        ...
        if layer_path.endswith(".lora_down.weight"):
            ...
            if lora_down_key in block_dict["attn"]:
                ...
                try:
                    ...
                    original_layer = QwenWeightHandlerLoRA._get_attention_layer(
                        transformer, block_idx, base_key
                    )
    ...
    def _process_ff_lora(
        ...
        if layer_path.endswith(".lora_down.weight"):
            ...
            if lora_down_key in block_dict["ff"]:
                ...
                try:
                    original_layer = QwenWeightHandlerLoRA._get_ff_layer(
                        transformer, block_idx, base_key
                    )
                        ...
                        for layer_name, lora_layer in block_weights["attn"].items():
                            if isinstance(lora_layer, QwenLoRALinear):
                                ...
                                if actual_attr_name and hasattr(block, actual_attr_name):
                                    ...
                                    print(f"         LoRA: A={lora_layer.lora_A.shape}, B={lora_layer.lora_B.shape}, scale={lora_layer.scale}")
                                else:
                                    print(f"      ❌ Cannot map layer {layer_name} to block attribute (mapped to: {actual_attr_name})")
                                    failed_count += 1
                            else:
