"""
Qwen Image Edit Prompt Rewriting Utilities

This module provides prompt rewriting functionality to improve the stability 
and quality of Qwen Image Edit results, based on the official demo recommendations.
"""
...
class QwenPromptRewriter:
    """
    Prompt rewriter for Qwen Image Edit that enhances prompts for better stability.
    
    This is a simplified version that doesn't require external API calls.
    For production use, consider integrating with Qwen's API for optimal results.
    """
    
    @staticmethod
    def detect_language(prompt: str) -> str:
        """Detect if the prompt is in Chinese or English."""
        chinese_ranges = [
    ...
    def enhance_prompt_en(original_prompt: str) -> str:
        """
        Enhance English prompts with additional details for better editing stability.
        
        This is a simplified version of the prompt enhancement logic.
        """
    ...
    def enhance_prompt_zh(original_prompt: str) -> str:
        """
        Enhance Chinese prompts with additional details for better editing stability.
    ...
    def enhance_edit_prompt(prompt: str, context: Optional[str] = None) -> str:
        """
        Enhance editing prompts with specific considerations for image editing tasks.
        
        Args:
            prompt: The original editing prompt
            context: Optional context about the image being edited
        
        Returns:
            Enhanced prompt for better editing stability
        """
    ...
    def create_edit_template(
        ...
        """
        Create a structured edit prompt template for common editing operations.
        
        Args:
            action: The editing action ('replace', 'add', 'remove', 'transform')
            target: What to target in the image
            replacement: What to replace it with (for replace actions)
            style: Optional style specification
            language: Language for the template ('en' or 'zh')
        
        Returns:
            Structured edit prompt
        """
        if language == 'zh':
            templates = {
