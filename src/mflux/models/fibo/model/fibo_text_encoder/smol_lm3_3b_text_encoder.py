...
class SmolLM3_3B_TextEncoder(nn.Module):
    ...
    def _build_attention_mask(attention_mask: mx.array) -> mx.array:
        ...
        mask_dtype = mx.float32  # Match PyTorch's mask dtype

        # Get minimum representable value for the dtype (like PyTorch's torch.finfo(dtype).min)
        min_dtype_value = mx.finfo(mask_dtype).min

        # Padding mask: 0 where attention_mask == 1, dtype.min where == 0
        # Match PyTorch: torch.where(mask, 0.0, torch.finfo(dtype).min)
        ...
        # Causal mask: dtype.min where j > i (masked), 0.0 otherwise
    ...
    def __call__(
        ...
        attention_mask_4d = self._build_attention_mask(attention_mask)
        ...
        # Final RMSNorm (matches model.norm in SmolLM3-3B)
        hidden_states = self.norm(hidden_states)

        if output_hidden_states:
            # Replace the last entry with the normalized hidden states
