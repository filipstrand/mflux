"""Joint attention for FLUX.2.

FLUX.2 joint attention is similar to FLUX.1 but with different dimensions:
- 48 heads (vs 24 in FLUX.1)
- 6144 hidden dim (vs 3072 in FLUX.1)
- Same 128 head dimension
"""
...
class Flux2JointAttention(nn.Module):
    """Joint attention for FLUX.2 joint transformer blocks."""
    ...
    def __init__(self):
        ...
        self.add_q_proj = nn.Linear(self.hidden_dim, self.hidden_dim, bias=False)
        self.add_k_proj = nn.Linear(self.hidden_dim, self.hidden_dim, bias=False)
        self.add_v_proj = nn.Linear(self.hidden_dim, self.hidden_dim, bias=False)
        self.to_add_out = nn.Linear(self.hidden_dim, self.hidden_dim, bias=False)

        # Normalization layers
        self.norm_q = nn.RMSNorm(self.head_dimension)
        self.norm_k = nn.RMSNorm(self.head_dimension)
        self.norm_added_q = nn.RMSNorm(self.head_dimension)
        self.norm_added_k = nn.RMSNorm(self.head_dimension)
    ...
    def __call__(
        ...
        # 1a. Compute Q,K,V for image hidden_states
        ...
        # 1b. Compute Q,K,V for text/context encoder_hidden_states
        ...
        # 1c. Concatenate for joint attention
