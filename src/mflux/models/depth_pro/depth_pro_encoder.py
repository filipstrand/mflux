...
class DepthProEncoder(nn.Module):
    def __init__(self):
        ...
        self.upsample_latent0 = [UpSampleBlock(dim_in=1152, dim_int=256, dim_out=256, upsample_layers=3) for i in range(4)]  # fmt: off
        self.upsample_latent1 = [UpSampleBlock(dim_in=1152, dim_out=256, upsample_layers=2) for i in range(3)]
        self.upsample0 = [UpSampleBlock(dim_in=1152, dim_out=512, upsample_layers=1) for i in range(2)]
        self.upsample1 = [UpSampleBlock(dim_in=1152, dim_out=1024, upsample_layers=1) for i in range(2)]
        self.upsample2 = [UpSampleBlock(dim_in=1152, dim_out=1024, upsample_layers=1) for i in range(2)]
    ...
    def __call__(self, x) -> list[mx.array]:
        ...
        x_pyramid_encodings = self.reshape_feature(x_pyramid_encodings, self.out_size, self.out_size)

        # Step 3: merging.
        x_latent0_encodings = self.reshape_feature(self.backbone_highres_hook0, self.out_size, self.out_size)
        x_latent1_encodings = self.reshape_feature(self.backbone_highres_hook1, self.out_size, self.out_size)
        ...
        for block in self.upsample_latent0:
            x_latent0_features = block(x_latent0_features)
        for block in self.upsample_latent1:
            x_latent1_features = block(x_latent1_features)
        for block in self.upsample0:
            x0_features = block(x0_features)
        for block in self.upsample1:
            x1_features = block(x1_features)
        for block in self.upsample2:
            x2_features = block(x2_features)
