    ...
    def cmd_coverage(self, script: str, output: Optional[str] = None, include_dirs: Optional[list[str]] = None):
        ...
        if include_dirs:
            ...
            for include_dir in include_dirs:
                ...
                # Track absolute include directories so we can match files that live
                # outside the project root (e.g., ComfyUI checkout on Desktop).
                try:
                    abs_dir = str(Path(include_dir).resolve())
                    if not abs_dir.endswith("/"):
                        abs_dir += "/"
                    include_patterns_abs.append(abs_dir)
                except Exception:  # noqa: BLE001
                    pass
        ...
        for file_path, lines in coverage_sets.items():
            ...
            try:
                ...
                if file_path_obj.is_absolute():
                    path_str = file_path_obj.as_posix()

                    # First try relative-to-project-root (default case)
                    try:
                        rel_path = file_path_obj.relative_to(project_root)
                        rel_path_str = rel_path.as_posix()

                        matches_pattern = any(rel_path_str.startswith(pattern) for pattern in include_patterns)
                        if matches_pattern:
                            mflux_coverage_sets[file_path] = lines
                        else:
                            # If not under any include pattern relative to project root, allow absolute
                            # include directories (external checkouts).
                            matches_abs = any(path_str.startswith(abs_pattern) for abs_pattern in include_patterns_abs)
                            if matches_abs:
                                mflux_coverage_sets[file_path] = lines
                            else:
                                excluded_count += 1
                        ...
                        # File is not under project root; fall back to absolute include dirs
                        matches_abs = any(path_str.startswith(abs_pattern) for abs_pattern in include_patterns_abs)
                        if matches_abs:
                            mflux_coverage_sets[file_path] = lines
                        else:
