...
class DebuggerService:
    ...
    def _validate_ab_testing_constraints(self, script_path: str) -> None:
        ...
        # Track which A/B names we see and where (list to detect duplicates)
        ab_locations: Dict[str, list[tuple[str, int]]] = {}
        ...
        def scan_file(file_path: str) -> None:
            try:
                ...
                for lineno, line in enumerate(lines, start=1):
                    ...
                    # Check for triple quotes and toggle state
                    if '"""' in line:
                        count = line.count('"""')
                    ...
                    # Skip if we're currently in a docstring (the closing line will toggle out, so it won't be skipped)
                    if in_docstring:
                        continue
                    ...
                    # Helper wrappers for A/B checkpoints (function calls only)
                    # Match: debug_checkpoint_mlx_A(...) but not: from ... import debug_checkpoint_mlx_A
                    if re.search(r"debug_checkpoint_mlx_A\s*\(", code_part):
                        if "mlx_A" not in ab_locations:
                            ab_locations["mlx_A"] = []
                        ab_locations["mlx_A"].append((abs_path, lineno))
                    if re.search(r"debug_checkpoint_mlx_B\s*\(", code_part):
                        if "mlx_B" not in ab_locations:
                            ab_locations["mlx_B"] = []
                        ab_locations["mlx_B"].append((abs_path, lineno))
                    if re.search(r"debug_checkpoint_pytorch_A\s*\(", code_part):
                        if "pytorch_A" not in ab_locations:
                            ab_locations["pytorch_A"] = []
                        ab_locations["pytorch_A"].append((abs_path, lineno))
                    if re.search(r"debug_checkpoint_pytorch_B\s*\(", code_part):
                        if "pytorch_B" not in ab_locations:
                            ab_locations["pytorch_B"] = []
                        ab_locations["pytorch_B"].append((abs_path, lineno))

                    # Direct debug_checkpoint(\"name\", ...) usage
                    if "debug_checkpoint" not in code_part:
                        continue
                    match = re.search(name_pattern, code_part)
                    if not match:
                        continue
                    name = match.group(1)
                    if name in ab_names:
                        # Treat this as an A/B checkpoint definition
                        if name not in ab_locations:
                            ab_locations[name] = []
                        ab_locations[name].append((abs_path, lineno))
                    else:
        ...
        try:
            ...
            # Important: Only enable strict A/B mode when the *user script*
            # (or its sibling scripts) declares A/B checkpoints. This avoids
            # surprising failures when library code contains A/B helpers for
            # targeted instrumentation, but the user is running a normal
            # (non A/B) debugging workflow.
            if not ab_locations:
                return
            ...
            # Check for duplicate A/B checkpoint definitions
            duplicates = []
            for name in ab_names:
                if name in ab_locations and len(ab_locations[name]) > 1:
                    locations_str = ", ".join([f"{fpath}:{line}" for fpath, line in ab_locations[name]])
                    duplicates.append(f"  - {name} defined at multiple locations: {locations_str}")

            if duplicates:
                raise ValueError(
                    "Duplicate A/B checkpoint definitions detected.\n"
                    "Each A/B checkpoint (mlx_A, mlx_B, pytorch_A, pytorch_B) must be defined exactly once.\n\n"
                    "Duplicates found:\n" + "\n".join(duplicates) + "\n\n"
                    "Please remove duplicate definitions and keep only one A/B checkpoint pair per framework."
                )
            ...
            if missing:
                details = []
                for name in sorted(ab_locations.keys()):
                    locations = ab_locations[name]
                    if locations:
                        fpath, line = locations[0]
