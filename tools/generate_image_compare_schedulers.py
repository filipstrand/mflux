...
def main():
    ...
    schedulers_to_test = {
        "linear": (LinearScheduler(), 14),
        "ddim": (DDIMScheduler(), 30),
        "euler_discrete": (EulerDiscreteScheduler(), 30),
    }

    # --- Generation Loop ---
    for name, (scheduler, steps) in schedulers_to_test.items():
        print(f"\n--- Generating with {name.upper()} Scheduler ---")
        image = flux.generate_image(
            seed=seed,
            prompt=TEST_PROMPT,
            config=Config(num_inference_steps=steps, height=1024, width=1024, guidance=2.5),
            scheduler=scheduler,
        )
