"""
Unit tests for Chroma LoRA mapping and loading.

Tests verify:
1. ChromaLoRAMapping covers all expected targets
2. ChromaLoRAMapping excludes modulation layers (norm1.linear, etc.)
...
class TestChromaLoRAMappingCoverage:
    """Tests for ChromaLoRAMapping target coverage."""

    @pytest.mark.fast
    def test_mapping_returns_non_empty_list(self):
        """Verify get_mapping returns a non-empty list of targets."""
        mapping = ChromaLoRAMapping.get_mapping()
...
class TestChromaLoRAMappingExclusions:
    """Tests for targets that should NOT be in ChromaLoRAMapping."""

    @pytest.mark.fast
    def test_no_norm1_linear_targets(self):
        """Verify norm1.linear is NOT included (Chroma uses DistilledGuidanceLayer)."""
        mapping = ChromaLoRAMapping.get_mapping()
        paths = [t.model_path for t in mapping]

        # These exist in FLUX but NOT in Chroma
        assert "transformer_blocks.{block}.norm1.linear" not in paths

    @pytest.mark.fast
    def test_no_norm1_context_linear_targets(self):
        """Verify norm1_context.linear is NOT included."""
        mapping = ChromaLoRAMapping.get_mapping()
        paths = [t.model_path for t in mapping]
    ...
    @pytest.mark.fast
    def test_no_single_block_norm_linear_targets(self):
        """Verify single block norm.linear is NOT included."""
        mapping = ChromaLoRAMapping.get_mapping()
        paths = [t.model_path for t in mapping]
...
class TestChromaLoRAMappingBFLPatterns:
    ...
    def test_single_block_linear2_patterns(self):
        ...
        # Check BFL pattern - linear2 doesn't need transforms
        bfl_patterns_found = False
        for target in proj_out_targets:
            if any("lora_unet_single_blocks" in p and "linear2" in p for p in target.possible_up_patterns):
                bfl_patterns_found = True
                # linear2 maps directly to proj_out, no transforms needed
        assert bfl_patterns_found


class TestChromaLoRAMappingAlphaPatterns:
...
class TestChromaLoRAMappingTargetCount:
    ...
    def test_total_target_count(self):
        ...
        # Joint blocks: 12 targets × 2 formats = 24
        # Single blocks: 5 targets × 2 formats = 10
        # Total: 34 (but some may be deduplicated)
        # Should be at least 30 targets
